...
if (!function_exists('getUMRONo')) {
    function getUMRONo()
    {
        ...
        // return $result;
         // Get the start number from environment variables
         $start = env('BABY_UMRO_START');
         ...
         // Get the maximum OP number for the current month
         $maxOPNo = \App\Models\Patient::select('op_no')->where('op_no', 'like', '%' . $formData . '%')->max('op_no');
 
         // Extract the sequence number from the maximum OP number
         $sequenceNumber = $maxOPNo ? (int)substr($maxOPNo, 4) + 1 : 1;
 
         // Format the UMR number as per the specified format
         $umrNo = $year . $month . str_pad($sequenceNumber, 3, '0', STR_PAD_LEFT);
        ...
        if ($data) {
            ...
            if ($data->input_type == 'checkbox') {
                ...
                foreach ($data->options as $option) {
                    ...
                    $html .= '<label class="checkbox doctor-color-black">
